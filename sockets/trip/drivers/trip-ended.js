  ...
  try {
    ...
    if (res) {
      if (res.status != TRIP_STATUS.COMPLETED) {
        ...
        if (res.driver.isEmployee) {
          setting.defaultCommission = 0
          setting.defaultRoadPickupCommission = 0
        }
        ...
        if (res.corporate) {
          ...
          if (corporate.pricing) {
            res.vehicleType.baseFare = corporate.pricing.baseFare
            res.vehicleType.pricePerKM = corporate.pricing.pricePerKM
            res.vehicleType.pricePerMin = corporate.pricing.pricePerMin
          }
        } else if (res.surge) {
        ...
        if (corp && corp.email) {
          const emailBody = await customerEmail({ trip: res, setting })
          sendEmail(corp.email, 'Corporate Trip summary', emailBody)
        }
        ...
        if (res.passenger) {
          ...
          if (setting.applyFirstTripIncentive && res.type !== TRIP_TYPES.ROAD_PICKUP && passengerTripCount === 1 && res.passenger.firstName != "_" && (tripDurationInMinutes >= 10 && tripDurationInMinutes < 180) && !res.dispatcher) {
            ...
            await session.withTransaction(async () => {
              await Incentive.create([{
                ...
                fare: res.fare,
                amount: setting.firstTripIncentiveAmount,
                reason: `First trip miles (${setting.firstTripIncentiveAmount} Birr)`
              }], { session: session })
              ...
              await User.updateOne({
                ...
                $inc: {
                  balance: setting.firstTripIncentiveAmount
                }
              }, { session: session })
            })
