...
module.exports = async (data, dispatcher, socket) => {
    ...
    try {
        ...
        if (data && data.pickUpAddress && data.dropOffAddress && data.vehicleType && data.employee) {
            ...
            if (!dispatcher.corporate) {
                return socket.emit('error', { message: "you must be a corporate type" })
            }

            let dispatcherCorporate = await Corporate.findOne({ _id: ObjectId(dispatcher.corporate) })

            if (!dispatcherCorporate) {
            ...
            let corporate = dispatcherCorporate._id

            let code = dispatcherCorporate.shortName + ':' + Math.random().toString(36).slice(2, 8)
            let found = false

            while (!found) {
                const ticket = await Ticket.findOne({ code })
                if (ticket) {
                    code = dispatcherCorporate.shortName + ':' + Math.random().toString(36).slice(2, 8)
                } else {
                    found = true
                }
            }
